...
function ExamStart() {
  ...
  useEffect(() => {
    const startExam = async () => {
      ...
      try {
        const response = await axiosInstance.get(`/examSheets/start?examId=${examId}&userId=${userId}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axiosInstance.post(
        `/exams/submit?examId=${examId}&userId=${userId}`,
        { answers }
      );
  ...
  return (
    <MainLayout>
    <div className="container mx-auto p-6 ml-11">
