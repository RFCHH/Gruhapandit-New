...
function Sidebar() {
  ...
  return (
    <div className="relative flex h-screen">
      <aside
        className={`fixed left-0 top-16 bottom-24 h-auto bg-white shadow-md rounded-2xl flex flex-col justify-between transition-all duration-300 ${isExpanded ? 'w-40' : 'w-14'
          }`}
      >
        <div className="flex items-center justify-center p-3">
          <img src={Logo} alt="Logo" className={`${isExpanded ? 'w-28 h-28' : 'w-12 h-12'}`} />
        </div>
          ...
          <ul>
            {menuItems.map((item) => (
              <li
                ...
                className={`flex items-center py-3 px-4 cursor-pointer  transition-all ${location.pathname === item.path ? 'bg-gray-200' : 'hover:bg-blue-100'
                  }`}
              >
                {item.icon}
                {isExpanded && <span className="ml-3 text-gray-700">{item.label}</span>}
              </li>
            ))}
          </ul>
        </nav>
        ...
        <div className="flex justify-end p-3">
          ...
          >
            {isExpanded ? <FaAngleDoubleLeft size={20} /> : <FaAngleDoubleRight size={20} />}
          </button>
        </div>
      </aside>
    </div>
