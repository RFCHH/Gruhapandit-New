...
import axiosInstance from "./LandingPage/AxiosInstance";

const LoginPage = () => {
  const [userId, setuserId] = useState("");
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [role, setRole] = useState("--select--"); // Role state to track dropdown selection
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formErrors.userId && !formErrors.password && !formErrors.role) {
      const formData = {
        userId,
        password,
        type: role.toUpperCase(), // Convert role to uppercase
      };
  
      try {
        const response = await axiosInstance.post(
          `/tuition-application/authentication/login`,
          formData
        );
